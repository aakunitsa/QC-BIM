...
! global Hessian
! -----------------------------------------------------
subroutine qc_hess_lr
    integer :: im
    nlr = 0

    do im = 1 + sys_me, nmol, sys_nproc
        call get_lr_hess(im) ! adds lr contributions to hessian
    end do
    call qc_mpi_barrier
    call qc_mpi_allreducei (nlr, nmol) ! count # of lr (debug)
end subroutine qc_hess_lr
        ...
        do is = 1, mol_nsite(imon)
            ...
            do bq_idx = 1, nbq(imon)
                ...
                do is_bq = 1, mol_nsite(ibq)
                    ...
                    if (bq_list(bq_idx, imon)%qm) then
                        if (.not. l_bq) cycle
                        call get_bqhess_ij(at_qm, at_bq, n, 1.0d0, k_esp) ! d(x_QM)d(y_BQ)
                        call get_bqhess_jj(at_bq, n, 1.0d0, k_esp) ! d(x_BQ)d(y_BQ) (does nothing if n!=0)
                    else
                        if (.not. l_bq) cycle
                        !call get_bqhess_ii(at_qm, -0.5d0, k_esp) ! d(x_QM)d(y_QM) (correct overcount)
                        !call get_bqhess_ij(at_qm, at_bq, n, 0.5d0, k_esp) ! (prevent overcount)
                        !call get_bqhess_ij(at_qm, at_bq, n, 0.5d0, k_esp) ! (prevent overcount)
                        if (cellzero(n)) then 
                            call get_bqhess_ii(at_qm, -0.5d0, k_esp) ! d(x_QM)d(y_QM) (correct overcount)
                            call get_bqhess_ij(at_qm, at_bq, n, 0.5d0, k_esp) ! d(x_QM)d(y_BQ)
                            call get_bqhess_jj(at_bq, n, 0.5d0, k_esp) ! d(x_BQ)d(y_BQ) (does nothing if n!=0)
                        else
                            call get_bqhess_ij(at_qm, at_bq, n, 1.0d0, k_esp) ! d(x_QM)d(y_BQ)
                            call get_bqhess_jj(at_bq, n, 1.0d0, k_esp) ! d(x_BQ)d(y_BQ) (does nothing if n!=0)
                        end if
...
subroutine qc_hess_dimers
    ...
    if (sys_nproc .gt. 1) then
        ! MASTER
        if (sys_master) then
            do idx_dimer = sys_nproc, num_pairs
        ...
        ! WORKER
            ...
            idx_dimer = sys_me
            if (idx_dimer .le. num_pairs) call get_dimer_hess(idx_dimer)
...
!------------------------------------------
! get_lr_hess: use BFS to run over all unit 
...
subroutine get_lr_hess (im)
    ...
    do while (.not. queue_empty(lat_vecs))
        ...
        call accumulate_hess_lr(im, vec, nhits)
        if (nhits .gt. 0) then
            if (a_dim) then
...
subroutine get_dimer_hess (idx_dimer)
    ...
    integer :: im, ip, jm, n(3), n0(3), nsite_i, nsite_j
    ...
    do is = 1, mol_nsite(im)
        ...
        do js = 1, mol_nsite(jm)
            ...
            call get_bqhess_ii(at_i, -1.0d0, k_esp)
            call get_bqhess_ij(at_i, at_j, n, -2.0d0, k_esp)
            call get_bqhess_jj(at_j, n, -1.0d0, k_esp)
            
            ! translate by -n...BQ interaction of j(0)i(-n)
            if ( (.not.cellzero(n)) .and. (im.ne.jm) ) then
                call get_bqhess_ii(at_j, -1.0d0, k_esp)
                call get_bqhess_ij(at_j, at_i, -n, -2.0d0, k_esp)
                call get_bqhess_jj(at_i, -n, -1.0d0, k_esp) 
            end if
        end do
    end do
...
subroutine accumulate_hess_lr(im, vec, nhits)
    ...
    do jm = 1, nmol
        ...
        if (rij2 < rcut_qq2) then
            ...
            ! Molecule jm(n) is in LR region of im(0)
            nhits = nhits + 1
            nlr(im) = nlr(im) + 1
            ...
            do is=1,mol_nsite(im)
                ...
                do js = 1,mol_nsite(jm)
                    ...
                    if (cellzero(n)) then
                        call get_bqhess_ii(ia+is, 0.5d0, k_esp)
                    else
                        call get_bqhess_ii(ia+is, 1.0d0, k_esp)
                    end if

                    ! -- i(0)j(n) (Upper right -K block IF n in range) --
                    ! -- j(n)i(0) (Lower left  -K block IF    n==0   ) --
                    if (cellzero(n)) then
                    ...
                    ! -- j(0)j(0) (Lower right K block IF n==0) --
                    if (cellzero(n)) then
                        call get_bqhess_jj(ja+js, n, 0.5d0, k_esp)
                    else
                        call get_bqhess_jj(ja+js, n, 1.0d0, k_esp)
                    end if
...
end subroutine accumulate_hess_lr
